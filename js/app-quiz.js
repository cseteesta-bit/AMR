(() => {
  ...
  A.clearQuizAutoAdvance=clearAutoAdvance;
  ...
  A.registerView('quiz',()=>{
    ...
    if(A.preferences.topic!=='all'&&!modules.some(module=>module.id===A.preferences.topic))A.preferences.topic='all';
  ...
  function advanceAfterAnswer(){
    ...
    autoAdvanceId=setTimeout(()=>{
      autoAdvanceId=null;
      const s=A.quizSession;
      if(!s||s.finished||A.currentRoute!=='quiz/play')return;
  ...
  A.registerAction('answerQuiz',button=>{
    clearAutoAdvance();
    if(Q.selectAnswer(A.quizSession,Number(button.dataset.option))){
      A.views['quiz/play']();
      advanceAfterAnswer();
    }
  });
  A.registerAction('quizPrevious',()=>{
    clearAutoAdvance();
    if(Q.move(A.quizSession,-1)){A.views['quiz/play']();scrollTo({top:0,behavior:'smooth'});}
  });
  A.registerAction('quizNext',()=>{
    clearAutoAdvance();
    if(Q.move(A.quizSession,1)){A.views['quiz/play']();scrollTo({top:0,behavior:'smooth'});}
  });
